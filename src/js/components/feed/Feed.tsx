...
class Feed extends BaseFeed {
  getSince() {
    if (this.state.settings.timespan !== "all") {
      return (
        Math.floor(Date.now() / 1000) - TIMESPANS[this.state.settings.timespan]
      );
    }
  }
  ...
  subscribeToFollows(since, callback) {
    ...
    const followedUsers = Array.from(
      SocialNetwork.followedByUser.get(myPub) || []
    );
