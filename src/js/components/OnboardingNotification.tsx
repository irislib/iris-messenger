...
function OnboardingNotification() {
  ...
  if (showFollowSuggestions) {
    content = renderFollowSuggestions();
  } else if (showNoIrisToAddress) {
    content = renderGetIrisAddress();
  } else if (noFollowers) {
    content = renderNoFollowers();
  }
  ...
  return null; // return null instead of empty string to indicate no output.
}
