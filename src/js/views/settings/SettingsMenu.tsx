...
if (
  ["iris.to", "beta.iris.to", "localhost"].includes(window.location.hostname)
) {
  ...
  render() {
    ...
    return (
      <>
        <div
          className={
            !this.props.activePage ? "settings-list" : "settings-list hidden-xs"
          }
        >
          {Helpers.isElectron ? (
            ...
            return (
              ...
              >
                <span class="text">
                  {t(SETTINGS[page])}
                  {page === "language" && (
                    <small className="mar-left5">{Icons.language}</small>
                  )}
                </span>
              </a>
            );
